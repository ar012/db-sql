...
total integer;
begin
select count(*)  into total from ac_domains;
return total;
end;
...
CREATE PROCEDURE passmessage(IN MSG varchar(100))
LANGUAGE SQL
P1:BEGIN
	INSERT INTO TESTMSG VALUES(MESG);
END P!;


CALL passmessage('How are you Today?')
...
-- Function: get_area_code(character varying)

-- DROP FUNCTION get_area_code(character varying);

CREATE OR REPLACE FUNCTION get_area_code(msisdn character varying)
  RETURNS character varying AS
$BODY$
declare result character varying;
begin
	select case 
		when left(msisdn, 10) in (select code from isd_rate where length(code) =10) then left(msisdn, 10)
		when left(msisdn, 9) in (select code from isd_rate where length(code) =9) then left(msisdn, 9)
		when left(msisdn, 8) in (select code from isd_rate where length(code) =8) then left(msisdn, 8)
		when left(msisdn, 7) in (select code from isd_rate where length(code) =7) then left(msisdn, 7)
		when left(msisdn, 6) in (select code from isd_rate where length(code) =6) then left(msisdn, 6)
		when left(msisdn, 5) in (select code from isd_rate where length(code) =5) then left(msisdn, 5)
		when left(msisdn, 4) in (select code from isd_rate where length(code) =4) then left(msisdn, 4)
		when left(msisdn, 3) in (select code from isd_rate where length(code) =3) then left(msisdn, 3)
		when left(msisdn, 2) in (select code from isd_rate where length(code) =2) then left(msisdn, 2)
		when left(msisdn, 1) in (select code from isd_rate where length(code) =1) then left(msisdn, 1)
	end 
	into result;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION get_area_code(character varying)
  OWNER TO ctsadmin;
